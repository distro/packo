...
class Package
	...
	def self.wrap (model)
		case model
			when Models::Repository::Package then Package.new(
				...
				features: (case model.type.to_sym
					when :binary then model.features
					when :source then model.features.map {|f| f.name}.join(' ')
				end),
			...
			when Models::InstalledPackage then Package.new(
				tags:     model.tags.map {|t| t.name},
				name:     model.name,
				version:  model.version,
				slot:     model.slot,
