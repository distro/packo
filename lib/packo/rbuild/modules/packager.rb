...
class Packager < Module
	...
	def self.pack (package, to = nil)
		...
		} or raise ArgumentError, 'package format unsupported'
	...
	def self.manifest (package, to = nil)
		format = @@formats.find {|format|
			(to || '.pko').end_with?(format.type)
		} or raise ArgumentError, 'package fromat unsupported'
	...
	def initialize (package)
		...
		package.stages.add :pack, method(:pack), at: :end, strict: true
	end

	def finalize
		package.stages.delete :pack
	end
	...
	def pack
		package.callbacks(:pack).do {
			...
			Packager.pack(package, "#{package.directory}/#{package.to_s :package}.#{(package.extension || '.pko').sub(/^[.]*/, '')}")
		}
	end
end
